...
| demo-hello：命令（含写操作示例） | ✅ 已实现 | `packages/demo-hello/index.cjs` L47-69 | lo-agent E2E（`lo-agent` 测试） |
| demo-hello：视图（HTML 快照） | ✅ 已实现 | `packages/demo-hello/index.cjs` L71-85 | lo-agent renderView 测试 |
| demo-hello：面板 / 编辑器 | ✅ 已实现 | `packages/demo-hello/index.cjs` L88-105 | lo-agent renderPanel/Editor 测试 |
| demo-hello：服务提供（`status-service`） | ✅ 已实现 | `packages/demo-hello/index.cjs` L107-118 | lo-agent 跨插件服务测试 |
| demo-hello：mountEl UI（isolated world） | ✅ 已实现 | `packages/demo-hello/ui/index.mjs` | lo-agent getUiModule + 真实链路冒烟 |
| demo-hello：config 默认值 | ✅ 已实现 | `packages/demo-hello/plugin.json` | ctx.config 测试 |
| demo-hello：写权限声明（operations.write） | ✅ 已实现 | `packages/demo-hello/plugin.json` | lo-agent 权限测试 |
| demo-consumer：服务消费 + 优雅降级 | ✅ 已实现 | `packages/demo-consumer/index.cjs` | lo-agent E2E |
| demo-consumer：dependsOn 依赖声明 | ✅ 已实现 | `packages/demo-consumer/plugin.json` | lo-agent 拓扑激活测试 |
